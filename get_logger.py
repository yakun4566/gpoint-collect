...
LOG_PATH = 'logs/gpoint-collect.log'  # 日志文件路径
LOG_LEVEL = 'INFO'  # 日志级别
# LOG_FORMAT = '%(levelname)s-%(asctime)s-[%(process)d]-%(filename)s-%(name)s-%(lineno)d-%(module)s-%(message)s'  # 每条日志输出格式
LOG_FORMAT = '%(asctime)s-%(levelname)s-%(process)d-[%(module)s-%(lineno)d]-%(message)s'  # 每条日志输出格式
...
if __name__ == '__main__':
    ...
    logger.info("发送 " + str(type) + " 类型数据，到:" + str(args[0]))